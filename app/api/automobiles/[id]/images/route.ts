...
export async function GET(
  ...
  try {
    ...
    console.error("GET /api/automobiles/[id]/images error:", error);
...
export async function POST(
  ...
  try {
    ...
    for (const file of files) {
      ...
      const relativePath = `/uploads/auto/${id}/${filename}`;

      const isPrimary = existingCount === 0 && sortOffset === 0;

      const img = await queryOne<AutoImage>(
    ...
    console.error("POST /api/automobiles/[id]/images error:", error);
