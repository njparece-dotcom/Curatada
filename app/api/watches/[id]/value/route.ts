...
import { WatchItem, WatchValuation, ComparableSale } from "@/lib/types";
...
export async function POST(
  ...
  try {
    ...
    const item = await queryOne<WatchItem>(
      `SELECT * FROM watch_items WHERE id = $1 AND user_id = $2`,
    ...
    const prompt = `You are a luxury watch and timepiece valuation expert. Research the current resale market value for: ${descriptor} in ${item.condition} condition.${item.movement ? ` Movement: ${item.movement}.` : ""}${item.case_material ? ` Case: ${item.case_material}.` : ""}${item.bracelet_material ? ` Bracelet: ${item.bracelet_material}.` : ""}
...
Include 3-8 comparable listings total (mix of sold and for-sale is fine).`;
    ...
    const valuation = await queryOne<WatchValuation>(
      `INSERT INTO watch_valuations (watch_item_id, valuation_type, price, notes, comparable_sales)
       VALUES ($1, 'ai', $2, $3, $4)
       RETURNING *`,
      [
        params.id,
    ...
    console.error("POST /api/watches/[id]/value error:", error);
