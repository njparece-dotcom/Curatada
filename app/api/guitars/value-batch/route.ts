...
import { GuitarCategory, GuitarItem } from "@/lib/types";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { category } = body as { category: GuitarCategory };
    ...
    const items = await query<GuitarItem>(
      `SELECT * FROM guitar_items WHERE category = $1 AND user_id = $2 ORDER BY created_at DESC`,
    ...
    for (const item of items) {
      try {
        const res = await fetch(`${baseUrl}/api/guitars/${item.id}/value`, {
          method: "POST",
          headers: { cookie },
        });
    ...
    console.error("POST /api/guitars/value-batch error:", error);
