/**
 * GET /api/auto-pursuits/findings
 *
 * Returns all findings for every auto pursuit, keyed by pursuit_id.
 * Optional ?id=uuid to limit to a single pursuit.
 */
...
export async function GET(req: NextRequest) {
  ...
  const rows = await query<PursuitFinding>(
    `SELECT pf.id, pf.pursuit_id, pf.source, pf.title, pf.url, pf.price, pf.condition,
     ...
     JOIN auto_pursuits ap ON ap.id = pf.pursuit_id AND ap.user_id = $1
     WHERE pf.pursuit_type = 'auto'
...
/**
 * DELETE /api/auto-pursuits/findings?id=<pursuit_id>
 * Clears all findings for the given auto pursuit owned by the current user.
 */
export async function DELETE(req: NextRequest) {
  ...
  await query(
    `DELETE FROM pursuit_findings WHERE pursuit_type = 'auto' AND pursuit_id = $1
     AND pursuit_id IN (SELECT id FROM auto_pursuits WHERE user_id = $2)`,
    [id, session.user.id],
  );
