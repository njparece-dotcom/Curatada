...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const pursuit = await queryOne<GuitarPursuit>(
      `SELECT * FROM guitar_pursuits WHERE id = $1 AND user_id = $2`, [id, session.user.id]
    );
  ...
  try {
    ...
    const pursuit = await queryOne<GuitarPursuit>(
      ...
      [brand||null,model||null,year_min||null,year_max||null,color_finish||null,
       ...
       facebook_location||null,exclude_terms||null,notes||null,status||"active",id,session.user.id]
    );
    if (!pursuit) return NextResponse.json({ error: "Not found" }, { status: 404 });
    return NextResponse.json(pursuit);
  } catch (e) {
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { id } = await params;
    const deleted = await queryOne(`DELETE FROM guitar_pursuits WHERE id=$1 AND user_id=$2 RETURNING id`, [id, session.user.id]);
