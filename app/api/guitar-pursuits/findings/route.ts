...
export async function GET(req: NextRequest) {
  ...
  // Scope findings to this user's pursuits
  const rows = await query<PursuitFinding>(
    `SELECT pf.id, pf.pursuit_id, pf.source, pf.title, pf.url, pf.price, pf.condition,
     ...
     JOIN guitar_pursuits gp ON gp.id = pf.pursuit_id AND gp.user_id = $1
     WHERE pf.pursuit_type = 'guitar'
...
export async function DELETE(req: NextRequest) {
  ...
  // Only delete if this pursuit belongs to the current user
  await query(
    `DELETE FROM pursuit_findings WHERE pursuit_type = 'guitar' AND pursuit_id = $1
     AND pursuit_id IN (SELECT id FROM guitar_pursuits WHERE user_id = $2)`,
    [id, session.user.id]
  );
