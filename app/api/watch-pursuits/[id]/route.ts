...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const pursuit = await queryOne<WatchPursuit>(
      `SELECT * FROM watch_pursuits WHERE id = $1 AND user_id = $2`, [id, session.user.id]
    );
  ...
  try {
    ...
    const pursuit = await queryOne<WatchPursuit>(
      ...
      [brand||null,model||null,reference_number||null,case_diameter||null,dial_color||null,materials||null,
       ...
       facebook_location||null,other_source||null,exclude_terms||null,notes||null,status||"active",id,session.user.id]
    );
    if (!pursuit) return NextResponse.json({ error: "Not found" }, { status: 404 });
    return NextResponse.json(pursuit);
  } catch (e) {
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { id } = await params;
    const deleted = await queryOne(`DELETE FROM watch_pursuits WHERE id=$1 AND user_id=$2 RETURNING id`, [id, session.user.id]);
