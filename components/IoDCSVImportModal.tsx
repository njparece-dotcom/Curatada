...
const REQUIRED_FIELDS: (keyof ColumnMap)[] = ["short_description", "category"];

const FIELD_ALIASES: Record<keyof ColumnMap, string[]> = {
  category:         ["category", "cat", "collection type"],
  short_description:["short_description", "title", "name", "description", "desc", "item name"],
  item_type:        ["item_type", "type", "kind", "object type"],
  brand:            ["brand", "artist", "maker", "author", "manufacturer", "creator"],
  long_description: ["long_description", "long description", "details", "full description"],
  year:             ["year", "yr", "date", "creation year"],
  condition:        ["condition", "cond", "grade", "state"],
  purchase_price:   ["purchase price", "price", "paid", "cost", "buy price", "value"],
  purchase_date:    ["purchase date", "date purchased", "bought date", "acquisition date"],
  purchase_source:  ["purchase source", "source", "seller", "gallery", "bought from"],
  provenance:       ["provenance", "history", "origin", "chain of custody"],
  notes:            ["notes", "note", "comments", "remarks"],
};

const TEMPLATE_CSV = `category,short_description,item_type,brand,year,condition,purchase_price,purchase_source,provenance,notes
fine-art,Abstract Landscape No. 4,Painting,Jane Doe,1987,Excellent,8500,Christie's Auction,"Purchased from artist estate",Signed lower right
memorabilia,1952 Mickey Mantle Baseball Card,Trading Card,,1952,Very Good,3200,eBay,"Private collector","PSA graded 4"`;
...
function validateRow(row: ParsedRow, map: ColumnMap): { valid: boolean; errors: string[] } {
  ...
  if (!get("short_description")) errors.push("Short description required");

  const cat = get("category");
  if (!IOD_CATEGORIES.includes(cat as IoDCategory))
    errors.push(`Invalid category "${cat || "(empty)"}". Must be: ${IOD_CATEGORIES.join(", ")}`);

  const cond = get("condition");
  if (cond && !CONDITIONS.includes(cond as never))
    errors.push(`Invalid condition "${cond}"`);

  const yr = get("year");
  if (yr) {
    const n = parseInt(yr);
    if (isNaN(n) || n < 1 || n > new Date().getFullYear() + 1) errors.push(`Invalid year "${yr}"`);
  }
...
export default function IoDCSVImportModal({ defaultCategory, onClose, onImportComplete }: IoDCSVImportModalProps) {
  const [step, setStep] = useState<Step>("upload");
  const [csvText, setCsvText] = useState("");
  const [headers, setHeaders] = useState<string[]>([]);
  const [rawRows, setRawRows] = useState<ParsedRow[]>([]);
  const [columnMap, setColumnMap] = useState<ColumnMap>({
    category: "", short_description: "", item_type: "", brand: "", long_description: "",
    year: "", condition: "", purchase_price: "", purchase_date: "", purchase_source: "",
    provenance: "", notes: "",
  });
  ...
  const processCSVText = useCallback((text: string) => {
    ...
    const newMap: ColumnMap = {
      ...
      item_type: autoMap.item_type ?? "",
      brand: autoMap.brand ?? "",
      long_description: autoMap.long_description ?? "",
      year: autoMap.year ?? "",
      condition: autoMap.condition ?? "",
      purchase_price: autoMap.purchase_price ?? "",
      purchase_date: autoMap.purchase_date ?? "",
      purchase_source: autoMap.purchase_source ?? "",
      provenance: autoMap.provenance ?? "",
      notes: autoMap.notes ?? "",
    };
    if (!newMap.category) {
  ...
  const runImport = async () => {
    ...
    for (let i = 0; i < validRows.length; i += BATCH) {
      ...
      try {
        const res = await fetch("/api/iod/import", {
          method: "POST",
    ...
    <div className="modal-backdrop fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="modal-content bg-surface border border-border rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl"
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-border sticky top-0 bg-surface z-10">
          <div>
            <h2 className="text-xl font-bold text-text">Import Collectibles from CSV</h2>
              ...
              <div className="bg-surface-2 rounded-xl p-4 border border-border flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-text-muted mb-2">
                    Required: <span className="text-text font-mono">short_description</span>.
                    Category defaults to <span className="text-text">{IOD_CATEGORY_LABELS[defaultCategory]}</span> if not provided.
                    Valid categories: {IOD_CATEGORIES.join(", ")}.
                  </p>
                  ...
                  <table className="w-full text-xs">
                    <thead className="sticky top-0 bg-surface-2 z-10">
                      <tr className="border-b border-border">
                        ...
                        <th className="px-3 py-2.5 text-left text-text-muted font-medium">Title</th>
                        <th className="px-3 py-2.5 text-left text-text-muted font-medium">Category</th>
                        <th className="px-3 py-2.5 text-left text-text-muted font-medium">Type</th>
                        <th className="px-3 py-2.5 text-left text-text-muted font-medium">Brand / Artist</th>
                    ...
                    <tbody>
                      {mappedRows.map((row, i) => (
                        <tr key={i} className={`border-b border-border last:border-0 ${!row._valid ? "bg-red-500/5" : ""}`}>
                          ...
                          <td className="px-3 py-2 text-text font-medium max-w-[200px] truncate">{row.short_description || <span className="text-red-400">—</span>}</td>
                          <td className="px-3 py-2 text-text-muted">{row.category || <span className="text-red-400">—</span>}</td>
                          <td className="px-3 py-2 text-text-muted">{row.item_type || "—"}</td>
                          <td className="px-3 py-2 text-text-muted">{row.brand || "—"}</td>
                ...
                <div className="space-y-1.5">
                  {mappedRows.filter(r => !r._valid).map((row, i) => (
                    <div key={i} className="text-xs text-text-muted bg-surface-2 rounded-lg px-3 py-2 border border-border">
                      <span className="text-red-400 font-medium">Row {row._rowIndex + 1}</span> ({row.short_description || "no title"}): {row._errors.join(", ")}
                    </div>
                  ))}
                </div>
              )}
